...
class ProfileInfo extends Component {
  ...
  async handleSave() {
    ...
    if (!witheSpace && !emailWrong && !passWrong) {
      ...
      // hacer el put
      await axios.put(`https://ezbrowser.herokuapp.com/quickmobility/user/update`,
        {
        ...
        .then(res => {
          if (res.status === 200) {
            Swal.fire(
              ...
              'Edición de la cuenta terminada',
              'success'
            )
            this.setState({ open: false });
          } else {
            Swal.fire(
              'Registro Fallido',
              'No ha sido posible registrar el usuario, intente nuevamente',
              'error'
            )
            this.setState({ open: false });
          }
  ...
  async componentDidMount() {
    ...
    await axios.get(`https://ezbrowser.herokuapp.com/auth/loggedUser/` + userLocalestorage.username,
      {
        headers: {
